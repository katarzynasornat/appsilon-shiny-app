...
timeline_ui <- function(id) {
  ...
  tagList(
    ...
    highchartOutput(ns("timeline_chart"), height = "200px")
...
timeline_server <- function(id, filtered_data) {
  moduleServer(id, function(input, output, session) {
    ...
    output$timeline_chart <- renderHighchart({
      ...
      highchart() %>%
        hc_chart(type = "column") %>%  # Changed from "line" to "column" for a bar chart
        hc_title(text = "") %>%  # Empty because we styled title above
        hc_xAxis(type = "datetime", title = list(text = "Date")) %>%  # X-Axis as DateTime
        hc_yAxis(title = list(text = "Observations")) %>%  # Add a Y-axis title
        hc_add_series(
          name = "Observations",
          data = list_parse2(hc_data %>%
                               mutate(period = datetime_to_timestamp(period)) %>%
                               select(period, total)),
          type = "column",  # Specify the series type as "column" for bars
          color = "#60A5FA"  # Bar color
        ) %>%
